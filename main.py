...
def train_model():
    ...
    # 3. Build training set based on US Open historical matches
    ...
    for _, row in us_open_matches.iterrows():
        ...
        # Randomize player positions to prevent target bias
        ...
        # Dynamic Elo Difference across all opponents
...
def predict_match(req: MatchRequest):
    ...
    if p1 not in player_stats or p2 not in player_stats:
        raise HTTPException(
            status_code=404, detail="One or both players not found in database."
        )

    p1_stats = player_stats[p1]
    p2_stats = player_stats[p2]

    # Real Elo difference calculated across all opponents
    ...
    if abs(p1_stats["total_matches"] - p2_stats["total_matches"]) > 15:
        more_exp = p1 if p1_stats["total_matches"] > p2_stats["total_matches"] else p2
        deciding_factors.append(
            f"{more_exp} has significantly higher ATP Tour match density"
        )
    if not deciding_factors:
        deciding_factors.append(
            "Even matchup: Decision based on subtle momentum differentials"
        )
    ...
    return {
        ...
        "h2h_diff": h2h_diff,
        "breakdown": {
            "pA_win_rate": round(p1_stats["win_rate"] * 100, 1),
            "pB_win_rate": round(p2_stats["win_rate"] * 100, 1),
